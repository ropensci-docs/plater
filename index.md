...
- [`read_plate()`](https://docs.ropensci.org/plater/reference/read_plate.md)
  takes data in plate layout form and converts it to a data frame, with
  one well per row, identified by well name.
- [`add_plate()`](https://docs.ropensci.org/plater/reference/add_plate.md)
  does the same thing, but merges the new columns into an existing data
  frame you provide.
...
to read them all in and combine them into a single data frame.
...
which takes a data frame with one well per row, and lays it out like
it’s on a plate.
...
`plater` is developed under a [Contributor Code of
Conduct](https://docs.ropensci.org/plater/CONDUCT.md). To contribute to
its development, you must agree to abide by its terms. Pull requests for
changes are accepted with gratitude. Please include tests as appropriate
